...
class Search extends Component
{
    ...
    public function render()
    {
        ...
        $files = MasterFile::with(['category', 'uploader'])
            ...
            ->where('status', 'active') // Only show latest (active) versions
            ->orderBy('created_at', 'desc')
            ->paginate(12);

        $categories = MasterFileCategory::withCount(['files' => function ($query) use ($userDepartment, $userEmail) {
            $query->when(! Auth::user()->hasRole(['administrator', 'developer']), function ($q) use ($userDepartment, $userEmail) {
                $q->where(function ($subQ) use ($userDepartment, $userEmail) {
        ...
        return view('livewire.document-library.search', compact('files', 'categories', 'departments'));
    }
}

...
class Analytics extends Component
{
    ...
    public function render()
    {
        ...
        $categoryStats = MasterFileCategory::select('document_categories.*')
            ->selectSub(function ($query) use ($userDepartment) {
                $subquery = $query->from('documents')
                    ->whereColumn('document_categories.id', 'documents.category_id')
                    ->where('status', 'active');
        ...
        return view('livewire.document-library.analytics', compact(

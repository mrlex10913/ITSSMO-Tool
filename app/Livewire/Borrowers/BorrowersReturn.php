...
class BorrowersReturn extends Component
{
    use WithPagination;
    ...
    public function updateBorrower($brfID){
        ...
        $this->brfLogs_returnremarks = $brfFind->return_remarks;
        $this->brfLogs_receivedcheckedby = $brfFind->recieved_checkedby;
        $this->brfLogs_retrunedBy = $brfFind->returnby;
        ...
        $this->brfLogs_items = $brfFind->itemBorrow->map(function ($item) {
            return [
                ...
                'return_remarks'=> $item->return_remarks
            ];
        })->toArray();
    ...
    {
        $brfReturn = BorrowerDetails::with(['itemBorrow.assetCategory'])
        ->where('status', 'Return')
